# ============================================================
# Exchange Rate multivariate forecaster (PyTorch + PennyLane)
# - Dataset: https://github.com/laiguokun/multivariate-time-series-data/tree/master/exchange_rate
#   (file: exchange_rate.txt.gz, daily rates vs USD, 8 series)
# - Multivariate [B, pred_len, F]
# - Train-only StandardScaler, early stopping, ReduceLROnPlateau
# - He (Kaiming) init + LayerNorm backbone
# - Learnable quantum-gated sigmoid (clamped)
# - Residual to last observation; report MAE/MSE on NORMALIZED data
# ============================================================
...
URL_GZ   = "https://raw.githubusercontent.com/laiguokun/multivariate-time-series-data/master/exchange_rate/exchange_rate.txt.gz"
PATH_GZ  = os.path.join(DATA_DIR, "exchange_rate.txt.gz")
...
# -------------------- Download dataset --------------------
def ensure_exchange(path_gz=PATH_GZ, url=URL_GZ):
    os.makedirs(os.path.dirname(path_gz), exist_ok=True)
    if not os.path.exists(path_gz):
        print("Downloading exchange_rate.txt.gz ...")
        urllib.request.urlretrieve(url, path_gz)
        print("Saved to", path_gz)
    else:
        print("exchange_rate.txt.gz already present at", path_gz)

ensure_exchange()

# -------------------- Load + normalization --------------------
df_raw = pd.read_csv(PATH_GZ, header=None, compression='gzip')
values = df_raw.values.astype(np.float32)   # [T, F]
T_total, F_raw = values.shape
...
# Standardize pe train only
scaler = StandardScaler()
X_all[:n_train] = scaler.fit_transform(X_all[:n_train])
X_all[n_train:] = scaler.transform(X_all[n_train:])

n_features = X_all.shape[1]
print(f"Splits -> train:{n_train}  val:{n_val}  test:{N - (n_train+n_val)}  | feats:{n_features}")
...
ds_train = WindowedDS(X_all, *idx_train, seq_len, pred_len)
ds_val   = WindowedDS(X_all, *idx_val,   seq_len, pred_len)
ds_test  = WindowedDS(X_all, *idx_test,  seq_len, pred_len)
...
class QTemporal(nn.Module):
    def __init__(self, n_features, k=128, d=128, g_min=0.05, g_max=0.95):
        super().__init__()
        # quantum params
    ...
    def forward(self, x):  # [B,T,F]
        ...
        for t in range(T):
            xt   = x[:, t, :]
            base = self.W(self.P(xt)) + self.b
            base = self.ln(base)
            h    = (1 - g_val) * h + g_val * base
        return h  # [B,d]

class QExchangeForecaster(nn.Module):
    def __init__(self, n_features, k=128, d=128, pred_len=24, drop_prob=0.1):
        super().__init__()
        self.backbone = QTemporal(n_features, k=k, d=d)
    ...
    def forward(self, x):  # [B,T,F]
        h = self.backbone(x)
        y_base = self.dec(h).view(x.size(0), self.pred_len, self.n_features)
        last_obs = x[:, -1, :].unsqueeze(1)   # [B,1,F]
        return y_base + last_obs

model = QExchangeForecaster(n_features=n_features, k=k, d=d, pred_len=pred_len, drop_prob=drop_prob).to(device)
